...
public class Question extends DataModel implements Serializable, Cloneable {
	...
	public Question fromJSON(JSONObject obj) {
		try {
			...
			for(int i=0; i<choices.length(); i++){
				add(new Choice().fromJSON(choices.getJSONObject(i)));
			}
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public static void main(String[] args) {
		...
		System.out.println(new Question().fromJSON(question.toJSON()).toJSON().toString().equals(question.toJSON().toString()));
	}
}
